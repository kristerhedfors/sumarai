...
def llamafile_service():
    ...
    start_result = subprocess.run(["python3", "sumarai.py", "--service"], capture_output=True, text=True)
    ...
    stop_result = subprocess.run(["python3", "sumarai.py", "--stop"], capture_output=True, text=True)
...
def test_start_llamafile_service(llamafile_service):
    ...
    result = subprocess.run(["python3", "sumarai.py", "--status"], capture_output=True, text=True)
...
def test_stop_llamafile_service():
    ...
    print(f"Service start stderr: {start_result.stderr}")
    # Stop the service
    result = subprocess.run(["python3", "sumarai.py", "--stop"], capture_output=True, text=True)
